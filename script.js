...
function loadText() {
  ...
  /*old method that excludes line break
  const matches = text.match(/(\w+|[.,?!;:'"’%*-—()]|[\n])/g);
  const words = matches || [];*/

  // Split text into sentences or chunks, keeping line breaks
  const sentences = text.split(/([.!?]|\n)/).filter(Boolean);

  let words = [];
  sentences.forEach(sentence => {
    // Extract words and punctuation from each sentence
    const matches = sentence.match(/(\w+|[.,?!;:'"’%*-—()])/g) || [];
    words = words.concat(matches);

    // If the sentence ends with a line break, add it to the words array
...
function countHighlightedSpan() {
  ...
  spanElements.forEach(span => {
    if (span.classList.contains("highlighted")) {
      selectedWordCount = selectedWordCount + 1
    }
  })
...
function retrieveTargetWords() {
  ...
  targetWords = Array.from(highlightedWords).map(span => span.innerText);
...
function showPopup(word, targetElement) {
  popupTW = word
  var popupSen = document.getElementById("exSenPopup");
  popupSen.innerHTML = "";
  ...
  popup.style.top = yCoordinate + window.innerHeight * 0.1 + "px";
...
function displayText(wordsArray, containerId) {
  ...
  wordsArray.forEach(function(word) {
    if (word === '\n') {
      // If the word is a line break, append a <br> element
      container.appendChild(document.createElement('br'));
    } else {
      // Otherwise, create a span for the word
      createSpan(word, container);
      container.appendChild(document.createTextNode(' ')); // Add a space after each word
    }
  });
}
...
function selectAllCheckboxes(checked, button) {
...
function SelectStarred() {
  ...
  const checkboxes = TWtableContainer.querySelectorAll('input[type="checkbox"]');

  // Get the table element by its ID
  const table = document.getElementById('targetTable');

  // Create an array to store the innerHTML values
  const wordCellContents = [];

  // Iterate through the rows in the table
  const rows = table.getElementsByTagName('tr');
  for (let i = 0; i < rows.length; i++) {
    // Get the wordCell in each row (second <td> element)
    const wordCell = rows[i].getElementsByTagName('td')[1]; // Index 1 is for the second <td>

    // Extract the innerHTML from the wordCell and push it to the array
    if (wordCell) {
      wordCellContents.push(wordCell.innerHTML);
    }
  }
  console.log(wordCellContents)
  for (var j = 0; j < starTW.length; j++) {
    for (var i = 0; i < checkboxes.length; i++) {
      if (starTW[j] == wordCellContents[i]) {
...
function checkAnswer() {
  ...
  //create and initialize an object to record how many times users make mistake for each TW
  if (Object.keys(mistakeRecord).length == 0) {
    answerList.forEach(element => {
      mistakeRecord[element] = 0;  // Use the answer as the key and initialize it with a value (e.g., 0)
    });
  }
  inputElements.forEach(function(input, index) {
    ...
    if (inputValue !== answer) {
      mistakeRecord[answer] = mistakeRecord[answer] + 1;
...
function showPracticeResult() {
  //show the practice result
  const blankPracticeResult = document.getElementById('blankPracticeResult');
  blankPracticeResult.style.display = "block";

  //create a table
  removeAllElement("resultTableContainer");
  ...
  // Create table headers
  var headers = [' ', '目標單字', '答錯次數']; // Replace with your header titles

  // Loop through the headers array and create <th> elements for each header
  ...
  //const MRLength = Object.keys(mistakeRecord).length
  // Generate table rows for target words
  for (let key in mistakeRecord) {
    if (mistakeRecord.hasOwnProperty(key)) {
      //add checkbox
      var row = document.createElement('tr');
      var checkboxCell = document.createElement('td');
      var checkbox = document.createElement('input');
      checkbox.type = 'checkbox';
      if (mistakeRecord[key] < 2) {
      ...
      //add TW
      var wordCell = document.createElement('td');
      wordCell.innerHTML = key;
      row.appendChild(wordCell);

      //add mistaken times
      var timesCell = document.createElement('td');
      timesCell.innerHTML = mistakeRecord[key];
...
function batchAddStar() {
  const resultTableContainer = document.getElementById("resultTableContainer")
  var checkboxes = resultTableContainer.querySelectorAll('input[type="checkbox"]');
  for (var i = 0; i < checkboxes.length; i++) {
    if (checkboxes[i].checked) {
      const TW = checkboxes[i].parentNode.nextSibling.innerHTML
      console.log(TW + " is being added to star.")
      if (!starTW.includes(TW)) {
        starTW.push(TW);
      }
    }
  }
